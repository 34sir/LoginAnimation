...
import com.example.chukc.loginanimation.R;
...
public class CircularProgressButton extends Button {
    ...
    private CircularBackgroundDrawable mAnimatedDrawableS;
	...
	/**
	 * 绘制蓝色背景以及白色圆环drawable
	 * @param canvas
     */
    private void drawIndeterminateProgress(Canvas canvas) {
	if (mAnimatedDrawableS == null) {
	    int offset = (getWidth() - getHeight()) / 2;
	    mAnimatedDrawableS = new CircularBackgroundDrawable(mColorIndicator, mStrokeWidth);
	    int left = offset + mPaddingProgress;
	    int right = getWidth() - offset - mPaddingProgress;
	    int bottom = getHeight() - mPaddingProgress;
	    int top = mPaddingProgress;
	    mAnimatedDrawableS.setBounds(left, top, right, bottom);
	    mAnimatedDrawableS.setCallback(this);
	} else {
	    mAnimatedDrawableS.draw(canvas);
	}
